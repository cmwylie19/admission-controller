...
CN = admission-controller.admission.svc
[ v3_req ]
...
[alt_names]
DNS.1 = admission-controller.admission.svc
EOF
...
openssl req -new -key webhook-server-tls.key -subj "/CN=admission-controller.admission.svc" -config server.conf \
